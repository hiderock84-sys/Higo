...
- `_headers` / `_redirects` を配信（セキュリティヘッダー + 旧URLから正規URLへ301）
...
1. `scripts/ensure-deps.mjs`（依存不足時は自動 `npm install`）
2. `vite build`
3. `scripts/prepare-pages-output.mjs`（互換出力・ルート別静的ページ生成）
4. `scripts/verify-generated-site.mjs`（自動検証）

`npm run verify:site` は `build:core` の後に同じ検証を実行するため、初回環境でもそのまま成功する構成です。
