...
const cleanupAssets = [
  ...
  path.join(projectRoot, 'dist', 'static', 'build-id.json'),
  ...targets.map((target) => path.join(target, 'static', 'logo-design.jpg')),
  ...targets.map((target) => path.join(target, 'static', 'build-id.json')),
]
...
async function syncStaticAssetsToDist() {
  ...
  await rm(path.join(distStaticDir, 'build-id.json'), { force: true })
